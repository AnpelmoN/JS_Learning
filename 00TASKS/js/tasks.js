...
const calculatedTotalBalance = (users) => {
  return users.reduce((acc, user) => acc + user.balance, 0);
};

console.log(calculatedTotalBalance(users));
...
const getSortedUniqueSkills = (users) => {
  const skills = [].concat(...users.map((user) => user.skills));
  const result = skills.filter((skill, index, arr) => {
